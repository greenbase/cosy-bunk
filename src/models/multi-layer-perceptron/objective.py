...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        structure_parameters = {
            ...
            # "activation_fn" : trial.suggest_categorical("activation_fn",[torch.nn.ReLU(),
            # torch.nn.Tanh()0]),
        ...
        # set up data loaders
        training_dataloader, validation_dataloader, test_dataloader = get_data_loaders(
            const.DATASET, training_parameters["batch_size_train"])

        # set up csv file for metrics of new model to be trained
        fieldnames = ["epoch", "distance_avg", "accuracy"]
        ...
        for epoch_count in range(1, epochs_total + 1):
            ...
            if epoch_count % 100 == 0 or epoch_count == epochs_total:
                ...
                metrics["distance_avg"], metrics["accuracy"] = test(validation_dataloader,
                                                                       model)

                # prune trial if loss 
                trial.report(metrics["distance_avg"], epoch_count - 1)
