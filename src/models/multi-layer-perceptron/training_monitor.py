...
app.layout = html.Div([
    html.Div([
        dcc.Graph(id="loss"),
        dcc.Interval(id="interval-component-loss", interval=10 * 1000, n_intervals=0)],
...
def update_loss(n):
    with open("model_metrics.csv", "r") as csv_file:
        df = pd.read_csv(csv_file, header=0, usecols=["epoch", "distance_avg"],
                         index_col="epoch")
    assert df.shape[1] == 1, f"Shape of loss df is {df.shape}"
    fig = px.line(df, y="distance_avg", )
    # fig.update_layout(margin=dict(pad=10))
    return fig
